...
"""
sss (for steady-state selection),           <- (estado estável) - EE
rws (for roulette wheel selection),         <- (roleta)         - R
sus (for stochastic universal selection),   <- (estocástico)    - E
rank (for rank selection),                  <- (truncamento)    - Tr
random (for random selection),              <- (aleatório)      - A
tournament (for tournament selection)       <- (torneio)        - To
"""
...
"""
single_point (for single-point crossover),  <- (ponto simples)  - PS
two_points (for two points crossover),      <- (dois pontos)    - DP
uniform (for uniform crossover),            <- (uniforme)       - U
scattered (for scattered crossover).        <- (espalhamento)   - Es
"""
...
swap (for swap mutation),                   <- (troca)          - T
inversion (for inversion mutation),         <- (inversão)       - I
scramble (for scramble mutation),           <- (embaralhamento) - Em
...
y = np.zeros((len(parent_selection_type), generations))            # --> mudar o array para avaliar outro fator
x = np.arange(generations)

# Variáveis utilizadas para os boxplots e arquivo
yTime = np.zeros((iteracoes, len(parent_selection_type)))            # --> mudar o array para avaliar outro fator
yBFit = np.zeros((iteracoes, len(parent_selection_type)))            # --> mudar o array para avaliar outro fator

for i in range(len(parent_selection_type)):                             # --> mudar o array para avaliar outro fator
    print("----------------------------Método {x}/{y}-------------------------------".format(x = i+1, y= len(parent_selection_type))) # --> mudar o array para avaliar outro fator
    i_global = i
    ...
    for it in range(iteracoes):
        ...
        ga_instance = pygad.GA(num_generations=generations,
                               ...
                               crossover_type=crossover_type,                       # --> mudar o array para avaliar outro fator
