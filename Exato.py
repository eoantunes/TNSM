...
percentualCobertura = [0.8, 0.9, 0.95, 0.98, 1]     # Porcentagem do número de clientes que devem ser atendidos (0 0.8, 0.9, 0.95, 0.98, 1)
...
for Interc in grauInterc:
    for consensoCobertura in percentualCobertura:
        ...
        # Restrição (1):  Somatório (mEM) Somatório (pEP) ymp.cmnp >= xn for all nEN
        #                 Somatório (mEM) Somatório (pEP) ymp.cmnp - xn >= 0 for all nEN
        ...
        for m in range(0, M):
            for p in range(0, P):
                ct = solver.Constraint(0, Ant, str(head))
                ct.SetCoefficient(Y[m2a(m, p, P)], -(Interc+1))
                head += 1
                for k in range(0, M):
                    for l in range(0, P):
