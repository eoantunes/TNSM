...
mutation_type = ['swap', 'inversion', 'scramble']

# Variáveis utilizadas para impressão dos gráficos de convergência
y = np.zeros((len(mutation_type), generations))            # --> mudar o array para avaliar outro fator
x = np.arange(generations)

# Variáveis utilizadas para os boxplots e arquivo
yTime = np.zeros((iteracoes, len(mutation_type)))            # --> mudar o array para avaliar outro fator
yBFit = np.zeros((iteracoes, len(mutation_type)))            # --> mudar o array para avaliar outro fator

for i in range(len(mutation_type)):                             # --> mudar o array para avaliar outro fator
    print("----------------------------Método {x}/{y}-------------------------------".format(x = i+1, y= len(mutation_type))) # --> mudar o array para avaliar outro fator
    i_global = i
    ...
    for it in range(iteracoes):
        ...
        ga_instance = pygad.GA(num_generations=generations,
                               ...
                               mutation_type=mutation_type[i],                      # --> mudar o array para avaliar outro fator
...
plt.plot(x, y[0], label='M-ALLOCATOR-T')
plt.plot(x, y[1], label='M-ALLOCATOR-I')
plt.plot(x, y[2], label='M-ALLOCATOR-Em')
...
t = np.asarray(yTime)
b = np.asarray(yBFit)
np.savetxt('result_Mut_time.txt', t, fmt="%f")
np.savetxt('result_Mut_bFit.txt', b, fmt="%d")